...
export default function RoomPage() {
  ...
  // Only redirect if we've waited a bit and still have nothing
  useEffect(() => {
    const id = setTimeout(() => {
      if (!room) router.push("/");
    }, 3000);
  ...
  function pickCategory(category: Category) {
    if (!room) return;
    socket.emit("player:set-category" as any, { roomId: room.id, category });
  }
      ...
      >
        ...
        <div className="text-center space-y-2">
          ...
          {isHost ? (
            <p className="text-emerald-400 font-bold text-sm">
              You are the host · Go to the host screen to start
            </p>
          ) : (
            <div className="flex items-center justify-center gap-2">
              <span className="w-2 h-2 bg-emerald-400 rounded-full animate-pulse" />
              <p className="text-white/45 font-bold text-sm">
        ...
        <AnimatePresence>
          {room.status === "category-select" &&
            ...
            !isHost && (
              ...
              >
                <p className="text-[11px] font-black text-white/35 uppercase tracking-widest">
                  Pick Your Category
                </p>
              ...
              {players.map((p, i) => (
                ...
                >
                  ...
                  <div className="min-w-0">
                    ...
                    {p.id === myId && (
                      <p className="text-[10px] text-violet-400 font-black">You</p>
                    )}
                  </div>
                </motion.div>
              ))}
        ...
        <div className="flex flex-col items-center gap-3 py-4">
          ...
          <p className="text-white/25 text-xs font-bold">
            {isHost ? "Head to the host screen to begin" : "Sit tight — the host will start soon"}
          </p>
        </div>
      </motion.div>
    </main>
